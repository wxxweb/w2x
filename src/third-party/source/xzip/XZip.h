...
//
XEXTERN ZRESULT ZipAdd(HZIP hz, const TCHAR *dstzn, void *src, unsigned int len, DWORD flags);
...
XEXTERN ZRESULT ZipGetMemory(HZIP hz, void **buf, unsigned long *len);
...
XEXTERN HZIP CreateZipZ(void *z,unsigned int len,DWORD flags);
XEXTERN ZRESULT CloseZipZ(HZIP hz);
XEXTERN unsigned int FormatZipMessageZ(ZRESULT code, char *buf,unsigned int len);
XEXTERN bool IsZipHandleZ(HZIP hz);
XEXTERN BOOL AddFolderContent(HZIP hZip, TCHAR* AbsolutePath, TCHAR* DirToAdd);
