...
#define W2X_FLASH_WND_CLASS TEXT("W2xFlashWndClass")
...
CWndFlashImpl::~CWndFlashImpl()
{
	...
	SAFE_DELETE_ARRAY(m_wnd_name_ptr);
}
...
HRESULT STDMETHODCALLTYPE CWndFlashImpl::FSCommand(_bstr_t _command, _bstr_t _args)
{
	//static w2x::events::CFlashEvent s_flash_event(
	//	w2x::events::CFlashEvent::EVENT_FS_COMMAND, _command, _args);
	
	//IF_NULL_ASSERT_RETURN_VALUE(m_event_disp.get(), S_FALSE);
...
{
	if (NULL != m_ext_api_xml_parser) 
	{
		bool result = m_ext_api_xml_parser->ParseExtApiXml(_args);
		m_lpControl->SetReturnValue(result ? TEXT("<true/>") : TEXT("<false/>"));
	}
...
bool CWndFlashImpl::SetWndName(LPCTSTR _name)
{
	IF_NULL_ASSERT_RETURN_VALUE(_name, false);

	const size_t nCount = _tcslen(_name) + 1;
	m_wnd_name_ptr = new TCHAR[nCount];
	if (NULL == m_wnd_name_ptr)
	{
		return false;
	}

	memcpy(m_wnd_name_ptr, _name, nCount * sizeof(TCHAR));
	return true;
}


LPCTSTR CWndFlashImpl::GetWndName(void) const
{
	return m_wnd_name_ptr;
}
